...
async function activate(context) {
  ...
  server.listen(localPort, hostIP, () => {
    console.log(
      `Interpreter switcher server listening on ${hostIP}:${localPort}`
    );
  });
...
function handleSetInterpreterRequest(req, res) {
  ...
  req.on("end", async () => {
    try {
      ...
      console.log(
        `Received request for interpreter: ${requestedPythonPath}, will set to: ${absolutePythonPath}`
      );
      ...
      try {
        ...
        if (!isPathInKnownEnvironments(absolutePythonPath, knownEnvironments)) {
          const knownEnvironmentsString =
            formatEnvironmentsAsList(knownEnvironments);
...
function handlePythonApiError(err, res) {
  const errorMessage = `Failed to get Python interpreter: ${
