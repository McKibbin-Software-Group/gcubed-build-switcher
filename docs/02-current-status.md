...
- The VS Code extension now has an environment-selection adapter in `vscode-extension/src/python/environmentSelector.js`. `interpreterHandler.js` uses the adapter rather than calling Microsoft Python APIs directly. The adapter prefers `ms-python.vscode-python-envs`, verifies selection with `getEnvironment(scope)`, and falls back to the legacy `ms-python.python` API when the new extension is missing, disabled, or unable to select the requested interpreter.
...
Additional validation on 2026-05-27 after the Python Environments selector slice:
